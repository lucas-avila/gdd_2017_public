...
namespace PagoAgilFrba.CRUDClient
{
    public partial class CreateClientForm : Form
    {
        ...
        {
            if (String.IsNullOrEmpty(txtDni.Text) || Util.convertStringToNumber(txtDni.Text) == null){
                MessageBox.Show("Debe ingresar un dni (solo numerico)", "Error");
                return false;
            }
            ...
            List<Client> result = StoreManager.getInstance().executeReadStore<Client>("sp_exists_client_email", new ClientMapper(), parameters);
